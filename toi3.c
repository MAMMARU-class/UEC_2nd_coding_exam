...
int main(void){
    ...
    // format
	fitting_format_pointsfile("data/toi3.csv", N, deg1, y_vec, coef_mat_d1);
	fitting_format_pointsfile("data/toi3.csv", N, deg2, y_vec, coef_mat_d2);
	...
	fprintf(gid," set xrange [-50:50]\n");
	fprintf(gid, " set yrange [-1000:1000]\n");
	fprintf(gid, "set xlabel 'x'\n");
	fprintf(gid, "set ylabel 'y'\n");
	fprintf(gid," plot 'data/toi3.csv' pt 1 title 'original points'\n");
